...
export function buildPoPdf(data: PoPdfData): Blob {
  ...
  // The "Project:" line is suppressed for ISGEC (its full company + GSTIN block
  // already prints in the delivery address); every other project still shows it.
  const hideProjectLine = (data.projectCode ?? data.projectName ?? "").trim().toLowerCase() === "isgec";
  if (!hideProjectLine && data.projectName) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7);
    doc.setTextColor(60, 60, 60);
